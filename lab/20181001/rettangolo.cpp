...
bool Rettangolo::checkDim(float dim) { return dim > 1.0 && dim < 10.0; }

Rettangolo::Rettangolo() {}
Rettangolo::Rettangolo(float w) { setWidth(w); }
...
void Rettangolo::setWidth(float w) {
  if (checkDim(w)) width = w;
}
void Rettangolo::setHeight(float h) {
  if (checkDim(h)) height = h;
}

float Rettangolo::side() const { return height * 2 + width * 2; }
float Rettangolo::area() const { return height * width; }