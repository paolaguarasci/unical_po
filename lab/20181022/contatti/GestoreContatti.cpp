...
void GestoreContatti::stampaContatti() {
  ...
  for (; iter != contatti.end(); iter++) std::cout << (*iter);
}
bool GestoreContatti::inserisciContatto(const Contatto &c) {
  std::list<Contatto>::iterator iter = contatti.begin();
  for (; iter != contatti.end(); iter++)
    if (((*iter).getCognome() == c.getCognome()) &&
        ((*iter).getNome() == c.getNome()) &&
        ((*iter).getEmail() == c.getEmail()) &&
        ((*iter).getTelefono() == c.getTelefono()))
      return false;
  contatti.push_back(c);
  return true;
}
int GestoreContatti::numeroContatti() { return contatti.size(); }
void GestoreContatti::ordinaContatti() {
  contatti.sort([](const Contatto &a, const Contatto &b) {
    return a.getCognome() < b.getCognome();
  });
}
bool GestoreContatti::eliminaContattoConRemove(const Contatto &c) {
  contatti.erase(std::remove(contatti.begin(), contatti.end(), c),
                 contatti.end());
  return true;
}
std::vector<std::string> GestoreContatti::trovaNumeri(
    const std::string &nome, const std::string &cognome) {
...
std::string GestoreContatti::trovaCognomePiuFrequente(int &freq) {
  ...
  for (; iter != contatti.end(); iter++) {
    succ++;
    if (succ != contatti.end() &&
        (*iter).getCognome() == (*succ).getCognome()) {
      tmpCounter++;
    } else {
      tmpCounter = 1;
    }
