...
bool Contatto::operator==(const Contatto &c) const { return this->nome == c.nome && this->cognome == c.cognome && this->email == c.email && this->telefono == c.telefono; }
...
bool Contatto::operator>(const Contatto &c) const
{
  ...
  if (nome > c.nome)
    return true;
  return false; //Oppure return (*this)!=c && !((*this)<c);
}
const string &Contatto::getCognome() const { return cognome; }
const string &Contatto::getEmail() const { return email; }
...
void Contatto::setTelefono(const string &telefono) { this->telefono = telefono; }