...
{
  os << "SEM: " << toPrint.semestre << " Corso: \"" << toPrint.nome << "\" Studenti iscritti: " << toPrint.size << endl;
...
Corso::Corso(const Corso &toCopy)
{
  ...
  for (int i = 0; i < size; i++)
    this->matricole[i] = toCopy.matricole[i];
}
Corso::~Corso()
{
  delete[] matricole;
...
const Corso &Corso::operator=(const Corso &rhs)
{
  if (this != &rhs)
  {
    ...
    matricole = new int[capacity];
    for (int i = 0; i < size; i++)
      this->matricole[i] = rhs.matricole[i];
  }
  return *this;
}
...
int Corso::operator[](int indice)
{
  return matricole[indice];
}
const int Corso::operator[](int indice) const
{
  return matricole[indice];
}
