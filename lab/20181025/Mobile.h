...
class Mobile {
 ...
 public:
  Mobile(string _nomeMobile = "", string _produttore = "", int _prezzo = 0,
         Tipo _tipo = TAVOLO, string _collaboratore = "")
      : nomeMobile(_nomeMobile),
        produttore(_produttore),
        prezzo(_prezzo),
        tipo(_tipo),
        collaboratore(_collaboratore) {}
  ...
  bool operator==(const Mobile &right) const {
    return nomeMobile == right.nomeMobile && produttore == right.produttore &&
           prezzo == right.prezzo && tipo == right.tipo &&
           collaboratore == right.collaboratore;
  }

  friend ostream &operator<<(ostream &out, const Mobile &x) {
    return out << "[" << x.nomeMobile << ", " << x.produttore << ", "
               << x.prezzo << ", " << x.tipo << ", " << x.collaboratore << "]";
  }
};
