...
void stampaElettrodomestici(
    const std::vector<Tripla<std::string, std::string, double>> &v);
bool aggiungiElettrodomestico(
    std::vector<Tripla<std::string, std::string, double>> &v,
    const Tripla<std::string, std::string, double> &t);
int numeroElettrodomestici(
    const std::vector<Tripla<std::string, std::string, double>> &v);
int capacitaElettrodomestici(
    const std::vector<Tripla<std::string, std::string, double>> &v);
void ordinaElettrodomestici(
    std::vector<Tripla<std::string, std::string, double>> &v);
Tripla<std::string, std::string, double>
restituisciElettrodomesticoInPosizioneCentrale(
    const std::vector<Tripla<std::string, std::string, double>> &v);
...
void ordinaElettrodomestici(
    std::vector<Tripla<std::string, std::string, double>> &v) {
  sort(v.begin(), v.end(),
       [](Tripla<std::string, std::string, double> const &a,
          Tripla<std::string, std::string, double> const &b) {
         if (a.getOne() != b.getOne()) {
           return a.getOne() < b.getOne();
         } else if (a.getTwo() != b.getTwo()) {
           return a.getTwo() < b.getTwo();
         } else {
           return a.getThree() < b.getThree();
         }
       });
}
Tripla<std::string, std::string, double>
